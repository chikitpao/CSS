...
class SymbolTable:
    ...
    def calculate_result(self, v_div_diff):
        print("## SymbolTable.key_symbols: ", self.key_symbols, "len:", len(self.key_symbols))
        if len(self.key_symbols) == 1:
            ...
            print("Difference with 2 terms:", free_symbols, " Difference:", diff)
            M = sp.Matrix(self.div, 1, lambda row, col: sp.cancel(v_div_diff[row, 0] / diff)) # Can be optimized for performance, but require more complex code
            ...
            i = len(self.key_symbols[key_symbols_keys[0]].members) # occurence of c_0
            j = self.v_div[self.key_symbols[key_symbols_keys[0]].index] - self.v_div[index_of_second_symbol_in_diff] # difference c_0 - c_i as n == d
            k = M[index_of_second_symbol_in_diff, 0] # factor of difference from previous step to current step
            ...
            if len(self.key_symbols) == 2:
                ...
                print(f"i: {i} # occurences of c_0 = mu_0; occurences of c_1 = mu_1")
                mu_0 = i
                mu_1 = self.div - mu_0
                print(f"j: {j} # difference c_0 - c_i as n = d")
                print(f"k: {k} # factor of difference from previous step to current step")
                d = self.div
                n = d   # for example
                print(f"# Formula for {key_symbols_keys[0]}: (2**n + (mu_1 * j * k**((n//d)-1)))// d")
                print(f"# Formula for {key_symbols_keys[1]}:  (2**n - (i * j * k**((n//d)-1))// d")
                formula2_0 = lambda n, d: (2**n + (mu_1 * j * k**((n//d)-1)))//d
                formula2_1 = lambda n, d: (2**n - (i * j * k**((n//d)-1))) // d
                print(f"# n = {n} => {key_symbols_keys[0]} = {formula2_0(n, d)}, {key_symbols_keys[1]} = {formula2_1(n, d)}")
            else: # len(self.key_symbols) == 3:
                print(f"i: {i} # occurences of c_0 = mu_0")
                print(f"j: {j} # difference c_0 - c_i as n = d")
                print(f"k: {k} # factor of difference from previous step to current step")
                mu_0 = i 
                mu_1 = len(self.key_symbols[self.sym_c[index_of_second_symbol_in_diff]].members)
                mu_2 = len(self.key_symbols[self.sym_c[row_index_with_3_symbols]].members)
                print(f"mu_1: {mu_1} # occurences of {self.sym_c[index_of_second_symbol_in_diff]}")
                print(f"mu_2: {mu_2} # occurences of {self.sym_c[row_index_with_3_symbols]}")
                o = v_div_diff[row_index_with_3_symbols, 0].coeff(key_symbols_keys[0])
                p = v_div_diff[row_index_with_3_symbols, 0].coeff(self.sym_c[index_of_second_symbol_in_diff])
                q = v_div_diff[row_index_with_3_symbols, 0].coeff(self.sym_c[row_index_with_3_symbols])
                diff_0 = self.v_div[0]-self.v_div[row_index_with_3_symbols]
                print(f"o (coefficient {key_symbols_keys[0]}): {o}")
                print(f"p (coefficient {self.sym_c[index_of_second_symbol_in_diff]}): {p}")
                print(f"q (coefficient {self.sym_c[row_index_with_3_symbols]}): {q}")
                print(f"diff_0 ({key_symbols_keys[0]}-{self.sym_c[row_index_with_3_symbols]} as n = d): {diff_0}")
                ...
                formula3_part1 = lambda n, d: j * k**((n//d)-1)
                # formula3_part2 = lambda n, d: self.v_div[row_index_with_3_symbols]
                formula3_part2 = lambda n, d: diff_0 * (-q)**((n//d)-1) - p * j * (k**((n//d)-2)) * (((2**(sp.log(-q, 2) - sp.log(k, 2)))**((n//d)-1)) - 1) // (2**(sp.log(-q, 2) - sp.log(k, 2)) - 1)
                ...
                formula3_0 = lambda n, d: (2**n + mu_1 * formula3_part1(n, d) + mu_2 * formula3_part2(n, d)) // d
                formula3_1 = lambda n, d: (2**n + (mu_1 - d) * formula3_part1(n, d) + mu_2 * formula3_part2(n, d)) // d
                formula3_2 = lambda n, d: (2**n + mu_1 * formula3_part1(n, d) + (mu_2 - d) * formula3_part2(n, d)) // d                
                n = d # for example
                print(f"# n = {n} => {key_symbols_keys[0]} = {formula3_0(n, d)}, {self.sym_c[index_of_second_symbol_in_diff]} = {formula3_1(n, d)}, {self.sym_c[row_index_with_3_symbols]} = {formula3_2(n, d)}")
                
                # Tests
                for ni in range(1, 6):
                    n = ni * d
                    print(f"# n = {n} => c0-{self.sym_c[index_of_second_symbol_in_diff]}: {formula3_part1(n, d)}, c0-{self.sym_c[row_index_with_3_symbols]}: {formula3_part2(n, d)}")

                M = create_matrix_from_solution_vector_sp(self.v_div)
                v_new = self.v_div
                for var in range(1, 6):
                    print(f"(matric calc.) m: {var}, c0-c1: {v_new[0]-v_new[1]}, c0-{self.sym_c[index_of_second_symbol_in_diff]}: {v_new[0]-v_new[index_of_second_symbol_in_diff]}")
                    v_new = M * self.v_div
         ...
         # Filter rule I'm working on:
            ...
            raise NotImplementedError(f"## Ignore div={self.div} for now. Key symbols count {len(self.key_symbols)}.")
