...
class CyclicSievingMatrix:
    def __init__(self, d, debug=False):
        ...
        self.M = sp.Matrix(d, d, lambda row, col: self.zeta **
                           ((row * (col + 1)) % d))
        # print("# Matrix:", self.M)
        ...
        # row: index for functions, col: index of coefficient
        self.factors = sp.Matrix(
            d, d, lambda row, col: self.zeta ** (((d-row)*col) % d))
    ...
    def print_sum_of_columns(self):
        ...
        for i in range(0, self.d):
            for j in range(0, self.d):
                ...
                v[i, j] = shrink_poly(v[i, j], self.zeta,
                                      self.d, self.divisors)
...
def test_calculation(n, div):
    ...
    print(
        f"# Same to result from calcluate_sum_subsets_logarithmic? {is_equal}, time elapsed={t_log}")
    return is_equal


def test_roots_of_unity():
    expressions = ["(-1)**(3/5)", "-(-1)**(3/5)", "(-1)**(1/5)",
                   "(-1)**(1/3)", "(-1)**(2/3)", "(-1)**(4/5)", "(-1)**(2/5)"]
...
def test_calculate_product_matrix(start, end_exclusive):
    for i in range(start, end_exclusive):
        ...
        print(
            f"i: {i}, calculate_product_matrix: {CPM}, equals: {is_equal}, t_CPM: {t_CPM}, t_PM {t_PM}")
        assert (is_equal)
